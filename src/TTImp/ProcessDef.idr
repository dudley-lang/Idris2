...
--         logC "declare.def.lhs" 5 $ do pure $ show $ indent {ann = ()} 2 $ pretty lhs
         ...
         let lhsMode = the ElabMode
                          (if trans
                          then InTransform
                          else InLHS mult)
         (lhstm, lhstyg) <- the (Core (Term vars , Glued vars)) $
             wrapErrorC (eopts elab) (InLHS fc !(getFullName (Resolved n))) $
...
checkClause {vars} mult vis totreq hashit n elab nest env
    (WithClause ifc lhs_in wval_raw mprf flags cs)
    = do (lhs, (vars'  ** (sub', env', nest', lhspat, reqty))) <-
             checkLHS False mult hashit n elab nest env ifc lhs_in
         let wmode
               = if isErased mult then InType else InExpr

         (wval, gwvalTy) <- wrapErrorC (eopts elab) (InRHS ifc !(getFullName (Resolved n))) $
                elabFun elab (eopts elab) n wmode nest' env' env sub' wval_raw Nothing
         clearHoleLHS
         ...
         -- JE TODO: When I replace elab with opts, typechecking loops forever
         log "declare.def.clause" 3 $ "Applying to with argument " ++ show rhs_in
         rhs <- wrapErrorC (eopts elab) (InRHS ifc !(getFullName (Resolved n))) $
             checkTermSub n wmode elab nest' env' env sub' rhs_in
                          (gnf env' reqty)
...
processDef elab nest env fc n_in cs_in
  ...
  where
    ...
    -- JE TODO: Had [] for options, do we use custom or default elab?
        ...
        = do itm <- unelabNoPatvars [] tm
             handleUnify
               (do ctxt <- get Ctxt
                   ...
                   setUnboundImplicits autoimp
                   (lhstm, _) <- elabTerm (record {eopts = []} elab) n (InLHS mult) (MkNested []) []
