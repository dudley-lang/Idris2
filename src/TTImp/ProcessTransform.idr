...
processTransform elab nest env fc tn_in lhs rhs
    = do tn <- inCurrentNS tn_in
         tidx <- resolveName tn
         (_, (vars'  ** (sub', env', nest', lhstm, lhsty))) <-
             checkLHS True top True tidx elab nest env fc lhs
         logTerm "transform.lhs" 3 "Transform LHS" lhstm
         let extElab = record {eopts = InTrans :: eopts elab} elab
         rhstm <- wrapError (InRHS fc tn_in) $
                       checkTermSub tidx InExpr extElab nest' env' env sub' rhs (gnf env' lhsty)
         clearHoleLHS
